...
def register_campaign_tools(mcp: FastMCP):
    ...
    # ============================================================================
    # URL Suffix Management
    # ============================================================================

    @mcp.tool()
    def google_ads_set_campaign_url_suffix(
        customer_id: str,
        campaign_id: str,
        url_suffix: str
    ) -> str:
        ...
        Args:
            customer_id: Customer ID (without hyphens)
            campaign_id: Campaign ID to update
            url_suffix: URL parameters to append (e.g., 'utm_source=google&utm_medium=cpc&sm_kw=bollards'). Pass empty string to clear.

        Returns:
            Success message with the applied suffix
        """
        with performance_logger.track_operation('set_campaign_url_suffix', customer_id=customer_id):
            try:
                client = get_auth_manager().get_client()
                campaign_manager = CampaignManager(client)

                result = campaign_manager.set_campaign_url_suffix(
                    customer_id, campaign_id, url_suffix
                )

                audit_logger.log_api_call(
                    customer_id=customer_id,
                    operation="set_campaign_url_suffix",
                    resource_type="campaign",
                    resource_id=campaign_id,
                    action="update",
                    result="success",
                ...
                if url_suffix:
                    return (
                        f"✅ Campaign {campaign_id} Final URL suffix updated!\n\n"
                        f"**Suffix:** `?{url_suffix}`\n\n"
                        f"All ad URLs in this campaign will now append these parameters."
                    )
                else:
                    return f"✅ Campaign {campaign_id} Final URL suffix cleared."

            except Exception as e:
                error_msg = ErrorHandler.handle_error(e, context="set_campaign_url_suffix")
                return f"❌ Failed to set campaign URL suffix: {error_msg}"

    @mcp.tool()
    def google_ads_get_url_suffixes(
        customer_id: str,
        ...
        Args:
            customer_id: Customer ID (without hyphens)
            campaign_id: Optional campaign ID to filter to a specific campaign. If omitted, returns all enabled campaigns.

        Returns:
            Formatted table of URL suffixes and tracking templates
        """
        with performance_logger.track_operation('get_url_suffixes', customer_id=customer_id):
            try:
                client = get_auth_manager().get_client()
                ...
                output = "## Campaign URL Suffixes\n\n"
                output += "| Campaign | ID | Final URL Suffix | Tracking Template |\n"
                output += "|----------|----|------------------|-------------------|\n"
                ...
                for row in campaign_response:
                    ...
                    template = c.tracking_url_template or "(none)"
                    if len(template) > 40:
                        template = template[:37] + "..."
                    output += f"| {c.name} | {c.id} | {suffix} | {template} |\n"
                    campaign_ids.append(str(c.id))

                # Query ad groups with suffixes
                if campaign_ids:
                    if campaign_id:
                        ag_filter = f"AND campaign.id = {campaign_id}"
                    else:
                        ag_filter = ""
                    ...
                    ag_response = ga_service.search(
                        customer_id=customer_id, query=ag_query
                    )

                    output += "\n## Ad Group URL Suffixes\n\n"
                    output += "| Campaign | Ad Group | ID | Final URL Suffix |\n"
                    output += "|----------|----------|----|------------------|\n"

                    has_ag_suffixes = False
                    for row in ag_response:
                        suffix = row.ad_group.final_url_suffix
                        if suffix:
                            has_ag_suffixes = True
                        display_suffix = suffix or "(inherits campaign)"
                        output += f"| {row.campaign.name} | {row.ad_group.name} | {row.ad_group.id} | {display_suffix} |\n"

                    if not has_ag_suffixes:
                        output += "\n*No ad groups have custom URL suffix overrides.*\n"

                return output

            except Exception as e:
                error_msg = ErrorHandler.handle_error(e, context="get_url_suffixes")
                return f"❌ Failed to get URL suffixes: {error_msg}"
