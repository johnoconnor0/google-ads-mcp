...
from managers.ad_group_manager import (
...
def register_ad_group_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    def google_ads_batch_set_ad_group_url_suffixes(
        customer_id: str,
        ad_group_suffixes: str
    ) -> str:
        """
        ...
        Args:
            customer_id: Customer ID (without hyphens)
            ad_group_suffixes: JSON string containing array of objects with 'ad_group_id' and 'url_suffix'. Example: [{"ad_group_id": "123", "url_suffix": "sm_kw=bollards"}, {"ad_group_id": "456", "url_suffix": "sm_kw=wheel-stops"}]

        Returns:
            Success message with count of updated ad groups
        """
        with performance_logger.track_operation('batch_set_ad_group_url_suffixes', customer_id=customer_id):
            try:
                mappings = json.loads(ad_group_suffixes)

                if not isinstance(mappings, list) or len(mappings) == 0:
                    return "❌ ad_group_suffixes must be a non-empty JSON array of objects with 'ad_group_id' and 'url_suffix'"

                for m in mappings:
                    if "ad_group_id" not in m or "url_suffix" not in m:
                        return "❌ Each mapping must have 'ad_group_id' and 'url_suffix' keys"

                client = get_auth_manager().get_client()
                ad_group_manager = AdGroupManager(client)

                result = ad_group_manager.batch_set_url_suffixes(
                    customer_id, mappings
                )

                audit_logger.log_api_call(
                    customer_id=customer_id,
                    operation="batch_set_ad_group_url_suffixes",
                    resource_type="ad_group",
                    action="update",
                    result="success",
                    details={'ad_group_count': len(mappings)}
                )

                get_cache_manager().invalidate(customer_id, ResourceType.AD_GROUP)

                output = f"✅ Batch URL suffix update completed!\n\n"
                output += f"**Ad Groups Updated**: {result['ad_groups_updated']}\n\n"
                output += "| Ad Group ID | URL Suffix |\n"
                output += "|-------------|------------|\n"
                for m in mappings:
                    output += f"| {m['ad_group_id']} | {m['url_suffix'] or '(cleared)'} |\n"

                return output

            except json.JSONDecodeError:
                return "❌ Invalid JSON in ad_group_suffixes parameter"
            except Exception as e:
                error_msg = ErrorHandler.handle_error(e, context="batch_set_ad_group_url_suffixes")
                return f"❌ Failed to batch set URL suffixes: {error_msg}"
