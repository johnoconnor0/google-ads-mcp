...
class AdGroupManager:
    ...
    def batch_set_url_suffixes(
        self,
        customer_id: str,
        mappings: list
    ) -> Dict[str, Any]:
        """
        Bulk set Final URL suffixes for multiple ad groups.

        Args:
            customer_id: Customer ID
            mappings: List of dicts with 'ad_group_id' and 'url_suffix'
        ...
        for mapping in mappings:
            ...
            ad_group.resource_name = ad_group_service.ad_group_path(
                customer_id, mapping["ad_group_id"]
            )
            ad_group.final_url_suffix = mapping["url_suffix"]
        ...
        logger.info(f"Batch updated URL suffixes for {len(mappings)} ad groups")

        return {
            "ad_groups_updated": len(mappings),
            "results": [r.resource_name for r in response.results],
            "message": f"Successfully updated URL suffixes for {len(mappings)} ad groups"
        }
