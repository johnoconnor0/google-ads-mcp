...
class AudienceManager:
    ...
    def create_user_list(
        ...
        Args:
            ...
            url_contains: For RULE_BASED lists, URL patterns to match (OR logic).
                         e.g. ["arcadium.com.au"] or ["/escape-rooms", "/pricing"]
        ...
        if list_type == UserListType.CRMBASED:
            ...
            # Build flexible rule with URL matching
            ...
            operand_info = self.client.get_type("FlexibleRuleOperandInfo")
            rule_info = self.client.get_type("UserListRuleInfo")

            # Build rule item group with URL patterns
            rule_item_group = self.client.get_type("UserListRuleItemGroupInfo")

            patterns = url_contains if url_contains else [""]
            for pattern in patterns:
                rule_item = self.client.get_type("UserListRuleItemInfo")
                rule_item.name = "url__"
                rule_item.string_rule_item.operator = (
                    self.client.enums.UserListStringRuleItemOperatorEnum.CONTAINS
                )
                rule_item.string_rule_item.value = pattern
                rule_item_group.rule_items.append(rule_item)

            rule_info.rule_item_groups.append(rule_item_group)
            operand_info.rule = rule_info
            flexible_rule.inclusive_operands.append(operand_info)
